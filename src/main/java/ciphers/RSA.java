...
public class RSA {
	...
	public synchronized BigInteger encrypt(final BigInteger message) {
		return message.modPow(publicKey, modulus);
	}

	public synchronized String decrypt(final String message) {
		return new String((new BigInteger(message)).modPow(privateKey, modulus)
				.toByteArray());
	}

	public synchronized BigInteger decrypt(final BigInteger message) {
		return message.modPow(privateKey, modulus);
	}

	/** Generate a new public and private key set. */
	public synchronized void generateKeys(final int bits) {
		final SecureRandom r = new SecureRandom();
		final BigInteger p = new BigInteger(bits / 2, 100, r);
		final BigInteger q = new BigInteger(bits / 2, 100, r);
		modulus = p.multiply(q);

		final BigInteger m = (p.subtract(BigInteger.ONE))
	...
	public static void main(final String[] args) {
		...
		final String text1 = "This is a message";
		System.out.println("Plaintext: " + text1);

		final String ciphertext = rsa.encrypt(text1);
