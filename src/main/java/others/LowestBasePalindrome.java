...
public class LowestBasePalindrome {
		...
		while (!foundBase) {
			...
			for (base = 2; base < num2; base++) {
				...
				while (num > 0) {
					...
					// continuing the process of getting the remainder. This is
					// done
					// until n is <=0 and the number in the new base is
					// obtained.
					digit = (num % base);
					num /= base;
					// If the digit isn't in the set of [0-9][A-Z] (beyond base
					// 36), its character
					// form is just its value in ASCII.

					// NOTE: This may cause problems, as the capital letters are
					// ASCII values
					// 65-90. It may cause false positives when one digit is,
					// for instance 10 and assigned
					// 'A' from the character array and the other is 65 and also
					// assigned 'A'.

					// Regardless, the character is added to the representation
					// of n
					// in the current base.
				...
				// Num is assigned back its original value for the next
				// iteration.
				num = num2;
				// Auxiliary method reverses the number.
				String reverse = reverse(newNum);
				// If the number is read the same as its reverse, then it is a
				// palindrome.
	...
	private static String base2base(String n, int b1, int b2) {
		...
		for (int i = 0; i < n.length(); i++) {
			...
			charB1 = n.charAt(i);
			// if it is a non-number, convert it to a decimal value >9 and store
			// it in charB2
