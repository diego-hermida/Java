...
public class MaxHeap implements Heap {
	...
	public MaxHeap(final List<HeapElement> listElements) throws Exception {
		maxHeap = new ArrayList<>();
		for (final HeapElement heapElement : listElements)
			if (heapElement != null)
	...
	// value - 1
	public HeapElement getElement(final int elementIndex) {
	...
	private double getElementKey(final int elementIndex) {
		return maxHeap.get(elementIndex - 1).getKey();
	}

	// Swaps two elements in the heap
	private void swap(final int index1, final int index2) {
		final HeapElement temporaryElement = maxHeap.get(index1 - 1);
		maxHeap.set(index1 - 1, maxHeap.get(index2 - 1));
		maxHeap.set(index2 - 1, temporaryElement);
	}

	// Toggle an element up to its right place as long as its key is lower than
	// its parent's
	private void toggleUp(int elementIndex) {
		final double key = maxHeap.get(elementIndex - 1).getKey();
	...
	private void toggleDown(int elementIndex) {
		...
		while (((2 * elementIndex) <= maxHeap.size()) && wrongOrder) {
			// Check whether it shall swap the element with its left child or
			// its right one if any.
			if (((2 * elementIndex) < maxHeap.size())
					&& (getElementKey((elementIndex * 2) + 1) > getElementKey(
							elementIndex * 2))) {
	...
	private HeapElement extractMax() {
		final HeapElement result = maxHeap.get(0);
		deleteElement(0);
		return result;
	}

	@Override
	public void insertElement(final HeapElement element) {
		maxHeap.add(element);
	...
	@Override
	public void deleteElement(final int elementIndex) {
		if (maxHeap.isEmpty())
			try {
		...
		if (getElementKey(elementIndex) > getElementKey(
				(int) Math.floor(elementIndex / 2)))
			toggleUp(elementIndex);
		// ... or down ?
		else if ((((2 * elementIndex) <= maxHeap.size()) && (getElementKey(
				elementIndex) < getElementKey(elementIndex * 2)))
