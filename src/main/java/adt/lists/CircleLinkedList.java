...
public class CircleLinkedList<E> {
	...
	// constructer for class.. here we will make a dummy node for circly linked
	// list implementation with reduced error catching as our list will never be
	// empty;
	...
	public E remove(final int pos) {
		if ((pos > size) || (pos < 0))
			// catching errors
			throw new IndexOutOfBoundsException(
					"position cannot be greater than size or negative");
		Node<E> iterator = head.next;
		// we need to keep track of the element before the element we want to
		// remove we can see why bellow.
		Node<E> before = head;
		for (int i = 1; i <= pos; i++) {
			iterator = iterator.next;
			before = before.next;
		}
		final E saved = iterator.value;
		// assigning the next referance to the the element following the element
		// we want to remove... the last element will be assigned to the head.
