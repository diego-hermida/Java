 ...
 * LinkedList and Link.
 *
 * A linked list is simplar to an array, it holds values. However, links in a
 * linked list do not have indees. With a linked list you do not need to
...
class DoublyLinkedList {
	 ...
	 */
	public void insertHead(final int x) {
		final Link newLink = new Link(x); // Create a new link with a value
											// attached to it
	 ...
	 *            Element to be inserted
	 */
	public void insertTail(final int x) {
		final Link newLink = new Link(x);
	...
	public Link deleteHead() {
		final Link temp = head;
		head = head.next; // oldHead <--> 2ndElement(head)
		head.previous = null; // oldHead --> 2ndElement(head) nothing pointing
								// at old head so will be removed
		if (head == null)
			tail = null;
		return temp;
	}
	 ...
	 * @return Link deleted
	 */
	public Link delete(final int x) {
		Link current = head;
	 ...
	 *            Element to be added
	 */
	public void insertOrdered(final int x) {
		final Link newLink = new Link(x);
		Link current = head;
		while ((current != null) && (x > current.value)) // Find the position to
															// insert
			current = current.next;
...
class Link {
	...
	public static void main(final String args[]) {
		final DoublyLinkedList myList = new DoublyLinkedList();
