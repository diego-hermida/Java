...
public class Queue {
	...
	public boolean insert(final int x) {
		 ...
		 * never happen.
		 *
		 * It's necessary to do it like this, but by the order of the code this does not
		 * do anything.
		 */
		if ((rear + 1) == (maxSize - 1)) // If the back of the queue is the end of the array wrap around to the front
			rear = -1;
		rear++;
		queueArray[rear] = x;
		nItems++;
