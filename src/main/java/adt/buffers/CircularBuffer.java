...
public class CircularBuffer {
	...
	public CircularBuffer(final int buffer_size) {
	...
	private boolean IsPowerOfTwo(final int i) {
		return (i & (i - 1)) == 0;
	}

	private int getTrueIndex(final int i) {
	...
	public boolean writeToCharBuffer(final char c) {
	...
	private static class TestWriteWorker implements Runnable {
		...
		public TestWriteWorker(final CircularBuffer cb) {
			_buffer = cb;
		}
	...
	private static class TestReadWorker implements Runnable {
		...
		public TestReadWorker(final CircularBuffer cb) {
			_buffer = cb;
		}
		...
		public void run() {
			...
			while (!Thread.interrupted()) {
				final Character c = _buffer.readOutChar();
				if (c != null)
	...
	public static void main(final String[] args) throws InterruptedException {
		...
		final CircularBuffer cb = new CircularBuffer(buffer_size);

		// create threads that read and write the buffer.
		final Thread write_thread = new Thread(new TestWriteWorker(cb));
		final Thread read_thread = new Thread(new TestReadWorker(cb));
		read_thread.start();
		write_thread.start();
